...
#     1) RESULT_API_URL in Streamlit secrets (production)
...
def load_remote_result(user_id: str, access_code: str) -> Tuple[Optional[dict], Optional[str], Optional[str]]:
    ...
    Expected endpoint response can be either the APB result JSON directly or:
        {"ok": true, "result": {...APB result...}}

    Configure in .streamlit/secrets.toml:
        RESULT_API_URL = "https://..."
        APB_SECRET = "..."                 # optional header
    """
    url = str(streamlit_secret("RESULT_API_URL", "") or "").strip()
    if not url:
    ...
    try:
        response = requests.post(
            url,
            headers=headers,
            json={"user_id": user_id.strip(), "access_code": access_code.strip()},
            timeout=20,
        )
...
def authenticate(user_id: str, access_code: str) -> Tuple[Optional[dict], Optional[str], Optional[str]]:
    ...
    if str(streamlit_secret("RESULT_API_URL", "") or "").strip() and error:
    ...
    if not local_result_files() and not str(streamlit_secret("RESULT_API_URL", "") or "").strip():
        return None, None, (
            "No result source is configured. For testing, place the generated *_Result.json "
            "beside this Streamlit app. For production, configure RESULT_API_URL in Streamlit secrets."
        )
...
def distribution_chart(rows: Iterable[dict]) -> None:
    ...
    if df.empty:
        ...
        return
    chart_df = df.set_index("Category")[["Actual %PF", "Recommended %PF"]]
