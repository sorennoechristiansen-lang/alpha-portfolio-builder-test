...
These choices are connected. Portfolio value, position size, number of stocks and sector diversification form a small puzzle that APB will try to solve throughout the following steps. You can use the **Previous** and **Next** buttons to move back and forth and adjust these basic rules as you refine sectors, regions, structure layers and other preferences.
    ...
    preset_col, apply_col, restore_col = st.columns([2.2, 1.0, 1.2])
    with preset_col:
        selected_structure_preset = st.selectbox(
            "Suggested structure",
            list(STRUCTURE_PRESETS.keys()),
            index=0,
            key="structure_preset_choice",
            help="Choose a starting structure. The percentages can still be adjusted manually afterwards.",
        )

    with apply_col:
        st.markdown("<div style='height: 1.78rem;'></div>", unsafe_allow_html=True)
        if st.button("Apply setup", use_container_width=True, key="apply_structure_preset"):
            for preset_key, preset_value in STRUCTURE_PRESETS[selected_structure_preset].items():
                st.session_state[preset_key] = float(preset_value)
                st.session_state["_" + preset_key] = float(preset_value)
            st.rerun()

    with restore_col:
        st.markdown("<div style='height: 1.78rem;'></div>", unsafe_allow_html=True)
        if st.button("Restore recommended", use_container_width=True, key="restore_structure_recommended"):
            for preset_key, preset_value in STRUCTURE_PRESETS["Healthy balance (recommended)"].items():
                st.session_state[preset_key] = float(preset_value)
                st.session_state["_" + preset_key] = float(preset_value)
            st.session_state["structure_preset_choice"] = "Healthy balance (recommended)"
            st.rerun()
