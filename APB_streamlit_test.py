...
set_default("active_order_blocked", False)
...
def check_active_order(email):
    try:
        response = requests.post(
            API_URL,
            headers={"X-APB-SECRET": api_secret()},
            json={"action": "check_active_order", "email": email},
            timeout=15,
        )
        if response.status_code == 200:
            result = response.json()
            return bool(result.get("active")), None
    ...
    with c2:
        ...
        if submitted:
            ...
            if not valid_email(clean_email):
                ...
                active, error = check_active_order(clean_email)
                st.session_state.logged_in = True
                st.session_state.user_email = clean_email
                st.session_state.active_order_blocked = active
                if error:
                    st.session_state.active_order_blocked = False
                st.rerun()
...
# Fresh login with an already-active order: block a second submission.
if st.session_state.active_order_blocked and not st.session_state.build_submitted:
    st.success("Your portfolio request is being processed.")
    st.write(
        "A new request cannot be submitted until your current Alpha Portfolio Builder "
        "request has been completed."
    )
    st.caption(
        "Please refer to the order number on your confirmation if you contact SIMGROVA about the request."
    )
    st.stop()
...
if step == 0:
    ...
    if st.session_state.build_submitted and st.session_state.submitted_order:
        ...
        st.info(
            "A new request cannot be submitted until this portfolio request has been completed."
        )
        st.stop()
    ...
    if st.button("BUILD PORTFOLIO", type="primary", use_container_width=True):
        try:
            ...
            if response.status_code == 200 and result.get("ok") is True:
                ...
                st.session_state.active_order_blocked = True
                st.error(
                    "An active portfolio request already exists for this email. "
                    "A new request can be submitted after the current request has been completed."
                )
