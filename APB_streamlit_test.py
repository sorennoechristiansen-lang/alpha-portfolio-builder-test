...
# Temporary secure connection test: Streamlit -> simgrova.dk
with st.expander("Connection test (temporary)"):
    st.caption("Tests secure delivery to the private APB storage on simgrova.dk.")
    if st.button("SEND TEST TO SIMGROVA", key="send_apb_test"):
        try:
            secret = st.secrets["APB_SECRET"]
            response = requests.post(
                "https://simgrova.dk/apb_api/receive.php",
                headers={"X-APB-SECRET": secret},
                json={"test": "Alpha Portfolio Builder", "source": "Streamlit"},
                timeout=15,
            )
            if response.status_code == 200 and response.json().get("ok") is True:
                st.success("Test sent successfully to simgrova.dk.")
            else:
                st.error(f"Server error: HTTP {response.status_code} – {response.text}")
