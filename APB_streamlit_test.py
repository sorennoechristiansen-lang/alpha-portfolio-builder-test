...
st.warning("TEMPORARY TEST: Secure connection from Streamlit to SIMGROVA private storage")
if st.button("SEND TEST TO SIMGROVA.DK", type="primary", use_container_width=True, key="send_apb_test"):
    try:
        ...
        if response.status_code == 200 and response.json().get("ok") is True:
            st.success("SUCCESS: Test data was sent to simgrova.dk.")
        else:
